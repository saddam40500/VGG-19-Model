...
# Configure the app
st.set_page_config(
    page_title = 'Tomato Leaf Disease Detection & Classification'
)
...
def main():
    ...
    st.title("Tomato Leaf Disease Detection and Classification")
    st.subheader("Select a diseased tomato leaf image to detect and classify the disease")
    image_file=st.file_uploader("Upload Image",type=["png","jpg","jpeg"])

    if image_file == None:
        st.warning("Please select a diseased tomato leaf image to detect and classify the disease")
    else:
        ...
        if st.button("Detect Disease"):
            ...
            x = random.randint(96,98)+ random.randint(0,99)*0.01
